...
var limitModels = []
module.exports.setModels = function setModels(array) {
  limitModels = array
}
...
function getConnections(connectionName) {
  var sourceFolder = process.cwd() + '/config/connections';
  var connections = require(sourceFolder);
  var result = {};
  if (connectionName) {
...
function getModelDefs(connection) {
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    var identity = fileName.toLowerCase();
    if (limitModels && limitModels.length && _.indexOf(limitModels, identity) === -1) {
      continue
    }
    try {
      if (files[i].indexOf('.js') > -1) {
        var model = require(sourceFolder + '/' + files[i]);
        console.log('found model', identity);
        model.globalId = fileName;
...
console.log('____', _.find(['user', 'user_account'], 'user'));
